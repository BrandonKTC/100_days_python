...
def count_down(count):
    ...
    canvas.itemconfig(
        timer_text, text=f"{count_min}:{count_sec if count_sec >= 10 else f'0{count_sec}'}")
    if count > 0:
        global timer_window
...
start_button = Button(text="Start", bg=YELLOW, padx=0,
                      pady=0, bd=0, highlightthickness=0, command=start_timer)
start_button.grid(column=0, row=2)

reset_button = Button(text="Reset", bg=YELLOW, padx=0,
                      pady=0, bd=0, highlightthickness=0, command=reset_timer)
reset_button.grid(column=2, row=2)

checkmarks = Label(text="✔"*checkmark, fg=GREEN, bg=YELLOW)
